...
Reader_csv() et Reader_excel() chargent le contenu dun fichier CSV ou Excel
dans un objet Espace. Les méthodes de celui-ci permettent de calculer les
différents indices Ttax, Ctax, Mtax.

Les fonctions de base pour calculer ces indices sont union() et intersection().
La fonction communes() sert à calculer les partitions (voir module "partition").
...
__all__=("Espace","Reader_csv","Reader_excel","ALGO_CTAX","ALGO_MRATIO",
         "Printer")
...
python_vers=python_vers.major*100+python_vers.minor
if python_vers<307:
    class nullcontext:
...
##    raise Exception(get_text("Python version >=3.7 requise",
##                             "Python version >=3.7 expected"))
del sys
...
'echantillons et 'codesespeces sont les liste des Echantillon et du code
...
"""
class Methode(dict):
    def __init__(self,nom,echantillons,codesespeces,source=None):
    ...
    clear=__delitem__=pop=popitem=setdefault=update=__setitem__
    ...
    @property
    def especes(self):
        if self.__especes is None:
            dd=defaultdict(set)
            for ech,esp in self.items():
                dd[esp].add(ech)
            for esp in dd:
                dd[esp]=frozenset(dd[esp])
            self.__especes=dd
        return self.__especes
...
class pMethode(Methode):
    def __init__(self,meth,mdict,an,eff_nom=None):
        mdict=mdict.copy()
        loc=[]
        codesp=[]
        all={}
...
class Espace(tuple):
    ...
    @property
    def paquets(self):
        if self.__paqs is None:
            self.__paqs=union(self)
        return self.__paqs
    ...
    sous la forme de couples (m,(a,b,c)), où 'm est le nom de la méthode et
    (a,b,c) le Rtax de cette méthode exprimé comme dans .rtax(). Les
    éléments retournés sont triés par ordre alphabétique des noms de méthodes.
    """
    def irtax(self):
...
class Printer:
    ...
    # match ratio.
    #
    def __prcommon(self,fractions,algo_ctax):
    ...
    redéfinir cette méthode. 'what a l'une des valeurs suivantes
        ...
        4   Début de la ligne. 'arg est un le numéro de la méthode, à compter
            de 0.
...
méthodes suivants :
    .methodes       Liste des Methode. None avant le chargement par .load().
    .echantillons   Liste des Echantillon utilisés dans les Methode. None
